  ...
  ALTER TABLE user_snoozes ENABLE ROW LEVEL SECURITY;
  CREATE POLICY "Users can view their own snoozes" ON user_snoozes
    FOR SELECT USING (auth.uid() = user_id);
  CREATE POLICY "Users can manage their own snoozes" ON user_snoozes
    FOR ALL USING (auth.uid() = user_id);
